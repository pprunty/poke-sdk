...
class SearchResource:
    ...
    def pokemon(
        ...
        *,
        ...
        if ability:
            ...
            ability_names = {p["pokemon"]["name"] for p in ability_data.get("pokemon", [])}
            names &= ability_names
    ...
    def generation(
        ...
        *,
        ...
        items = [NamedAPIResource(name=r["name"], url=r["url"]) for r in page.get("results", [])]
        ...
        if region:
            ...
            for r in items:
                ...
                g = self._client.generation.get(gen_id_or_name)  # Get full generation data
                if g.main_region.name.lower() == region.lower():
                    filtered.append(NamedAPIResource(
                        name=g.name, 
        ...
        *,
        ...
        if region:
            ...
            for r in items:
                # Extract generation name/id from the item
                gen_id_or_name = r.name
                g = await self._client.generation.get(gen_id_or_name)  # Get full generation data
                if g.main_region.name.lower() == region.lower():
                    filtered.append(NamedAPIResource(
                        name=g.name, 
                        url=f"{self._client.base_url}/generation/{g.id}"
