...
MEWTWO_SPECIES = {
    ...
    "base_happiness": 0,
    "hatch_counter": 30,
    "growth_rate": {"name": "slow", "url": "https://pokeapi.co/api/v2/growth-rate/1/"},
    "egg_groups": [
    ...
    "evolution_chain": {"url": "https://pokeapi.co/api/v2/evolution-chain/83/"},
}
    ...
    try:
        with respx.mock(assert_all_called=False) as router:
            ...
            detail = client.pokedex.detail(
                pokedex="kanto", number=150, version_group="red-blue"
            )
...
async def test_pokedex_detail_async():
    ...
    async with AsyncPoke() as client:
        with respx.mock(assert_all_called=False) as router:
            ...
            fighting_damage = next(
                (d for d in detail.damage_taken if d.type == "fighting"), None
            )
...
def test_version_group_auto_selection():
    ...
    try:
        with respx.mock(assert_all_called=False) as router:
            ...
            level_up_moves = [
                m for m in detail.level_up_moves if m.version_group == "red-blue"
            ]
