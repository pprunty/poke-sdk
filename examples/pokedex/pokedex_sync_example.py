...
def main():
    ...
    client = Poke()
        ...
        print(f"📋 Top 5 Johto Pokemon (by Total Base Stats):")
        print()
        
        for i, pokemon in enumerate(johto_rankings[:5], 1):
            print(f"{i}. {pokemon.name.title():<12} BST: {pokemon.total_base_stat:3d} Types: {'/'.join(pokemon.types).title()}")
        ...
        mewtwo_json = {
            ...
            "resistances": [
                d.type for d in mewtwo_detail.damage_taken if d.multiplier < 1.0 and d.multiplier > 0
            ],
            "movepool_size": {
        ...
        print(f"💡 Demonstrates sync API usage with generation parameters")
